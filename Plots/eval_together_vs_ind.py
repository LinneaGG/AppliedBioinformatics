...
truegt <>
truegl <>
imp1 <path/to/file/imputed/with/Beagle_together>
imp2 <path/to/file/imputed/with/Beagle_individually>
date <>
rollwin <>
bins <>
compute <>
...
def rollquants(dX: pd.DataFrame, dS1: pd.Series, dS2: pd.Series) -> pd.DataFrame:
    ...
    pctY1['dataset'] = ['beagle together'] * pctY1.shape[0]

    pdf2 = qual.QuantilesDataFrame(dX,
                                   dS2,
                                   bins_step=bS)
    pctY2 = pdf2.binnedX_rolling_quantilY(rollwin=rQ)
    pctY2['dataset'] = ['beagle individually'] * pctY2.shape[0]
...
if compute:
    for metric, d in metrics.items():
        if d is not None:
            ...
            pctY_comp['mean'] = pctY_comp['dataset'].apply(lambda x: yS_beagle.mean() if x == 'beagle together' else yS_phaser.mean())
...
for dquant, f in dataquants.items():
    if f is not None:
        ...
        for i, dset in enumerate(['beagle together', 'beagle individually']):
        ...
        labels[-2] = '{} (mean = {:.5f})'.format(labels[-2], meanf['beagle together'])
        labels[-1] = '{} (mean = {:.5f})'.format(labels[-1], meanf['beagle individually'])
