...
def rollquants(dX: pd.DataFrame, dS1: pd.Series) -> pd.DataFrame:
    ...
    pctY1['dataset'] = ['phaser'] * pctY1.shape[0]
...
qphasergt = qual.QualityGT(truegt, imputed_beagle, 0, idx='chrom:pos')
qphasergl = qual.QualityGL(truegl, imputed_beagle, 0, idx='chrom:pos')

print('\r\n{} variants from {} samples read from {}'.format(len(qphasergt.trueobj.variants),
                                                            len(qphasergt.trueobj.samples),
                                                            os.path.basename(truegt)))
print('\r\n{} variants from {} samples read from {}'.format(len(qphasergt.imputedobj.variants),
                                                            len(qphasergt.imputedobj.samples),
                                                            os.path.basename(imputed_beagle)))
if compute:
    bgldiff = qphasergt.diff()


print('\r\n{} variants from {} samples read from {}'.format(len(qphasergl.trueobj.variants),
                                                            len(qphasergl.trueobj.samples),
                                                            os.path.basename(truegl)))

mafS = qphasergt.trueobj.maf  # maf_info
...
if compute:
    metrics = {'precision_score': qphasergt.precision,
               'recall_score': qphasergt.recall,
               'f1_score':  qphasergt.f1_score,
               'concordance': qphasergt.concordance(),
               'allelic_dos': None,
               'cross_entropy': qphasergl.cross_entropy
               }
...
conc_list = qphasergt.concordance().tolist()
...
cross_list = qphasergl.cross_entropy.tolist()
...
for dquant, f in dataquants.items():
    ...
    if f is not None:
        ...
        for i, dset in enumerate(['phaser']):
